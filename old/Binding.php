...
readonly class Binding
{
    ...
    {
        $interactsWithLine = Str::is(
            sprintf($definition = '%s on line %s', '*}', '*'),
            $value
        );

        $code = Str::between($value, $closurePrototype = 'closure:', '}');

        $file = $this->getFile() ?: 'unknown-file';
        $line = $this->getLine() ?: 'unknown-line';

        if ($interactsWithLine) {
            $code = Str::after($value, $closurePrototype);

            $line = Str::afterLast($value, Str::space()) |> Number::parseInt(...) |> $this->transformLine(...);
        }

        $definition = sprintf(
            $interactsWithLine ? $definition : '%s:%s',
            $file,
            $line,
        );
        ...
        return Str::replace([$code, $boundPrototype], [$definition, $closurePrototype], $value);
    }
